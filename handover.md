...
| v0.19.0 | - | プラグイン非同期実行、差分統計常時表示、エンコーディング/行末文字のステータスバー表示、タブごとのdiffオプション独立化、フォルダ比較サイズ・日付フィルタ |
...
2. **行レベル編集の強化** — 複数行選択、ブロック単位のコピー/削除
3. **差分ナビゲーション強化** — 特定ステータス（Added/Removed/Modified）のみジャンプ
